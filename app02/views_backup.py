...
def info_xunjian(request):
    try:
        ...
        return JsonResponse({'success': 'success', 'message': f'已完成{xunjian_all_device}台设备巡检,共执行{xunjian_all_func}个函数。'}, status=200)
...
def info_history(request):
    ...
    return render(request, "info_history.html", {"row_obj13": row_obj13,'tag':tag})
