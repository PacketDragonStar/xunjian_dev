...
class fbouncModelForm(BootstrapModelForm):
    class Meta:
        model = models.function_group_relationship_table
        fields = '__all__'
        exclude = ['conn_timeout', 'timeout']


def fbounc_list(request):
    """ 设备列表 """
    search_fields = ['func', 'group_id']
    ...
    # 根据搜索条件去数据库获取
    queryset = models.function_group_relationship_table.objects.filter(query)

    # queryset = models.Devices.objects.all()
    form = fbouncModelForm()
    ...
    return render(request, 'fbounc_list.html', context)


@csrf_exempt
def fbounc_add(request):
    """ 新增、修改设备Ajax请求 """
    form =fbouncModelForm(request.POST)
    if form.is_valid():
        form.save()
...
@csrf_exempt
def fbounc_delete(request):
    """ 设备删除 """
    uid = request.GET.get('uid')
    exists = models.function_group_relationship_table.objects.filter(id=uid).exists
    if not exists:
        return JsonResponse({"status": False, "error": "数据不存在"})
    models.function_group_relationship_table.objects.filter(id=uid).delete()
    return JsonResponse({"status": True})


def fbounc_multi(request):
    """ 批量上传 """
    ...
    if request.method == 'POST' and 'dev' in request.FILES:
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                models.function_group_relationship_table.objects.update_or_create(func=func, group_id=group_id)
            return redirect('/fbounc/list/')
